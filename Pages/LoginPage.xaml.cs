...
namespace PriemPatient.Pages
{
    public partial class LoginPage : Page
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(LogId.Text) || string.IsNullOrWhiteSpace(LogPass.Text))
            {
                MessageBox.Show("Все поля обязательны для заполнения");
                return;
            }

            if (!int.TryParse(LogId.Text, out int userId))
            {
            ...
            try
            {
                ...
                if (user != null && user.Password == LogPass.Text)
                {
